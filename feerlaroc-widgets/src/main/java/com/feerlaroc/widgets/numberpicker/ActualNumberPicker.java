...
import com.feerlaroc.widgets.BuildConfig;
import com.feerlaroc.widgets.R;
import com.feerlaroc.widgets.numberpicker.listener.OnNumberClickedListener;
import com.feerlaroc.widgets.numberpicker.listener.OnValueChangeListener;
...
public class ActualNumberPicker extends View {
    ...
    private Rect mRightRectangle = new Rect();
    private Rect mLeftRectangle = new Rect();
    private Rect mNumberRectangle = new Rect();
    ...
    private OnNumberClickedListener mOnNumberClickedListener;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mInterval = attributes.getInt(R.styleable.ActualNumberPicker_interval, mInterval);
        mFastInterval = attributes.getInt(R.styleable.ActualNumberPicker_interval, mFastInterval);
    ...
    /**
     * Sets the {@link OnNumberClickedListener} to this number picker.
     *
     * @param listener Which listener to set
     */
    public void setListener(OnNumberClickedListener listener) {
        mOnNumberClickedListener = listener;
    }
     ...
     * @param requestedWidthMode An integer constant from {@link MeasureSpec} that declares measuring mode
     ...
     * Calls {@link OnValueChangeListener#onValueChanged(int, int)}, but posts it to the main looper.
     */
    private void notifyListener(final int newValue) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mOnNumberClickedListener != null) {
                    mOnNumberClickedListener.onNumberClicked( newValue);
                }
            }
        });
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mSelectedControl != CONTROL_NONE) {
                    ...
                    if (mLeftRectangle.contains((int)event.getX(), (int)event.getY())) {

                        onControlClicked(ARR_LEFT);
                    }else if(mRightRectangle.contains((int)event.getX(), (int)event.getY())){

                        onControlClicked(ARR_RIGHT);
                    }else if(mNumberRectangle.contains((int)event.getX(), (int)event.getY())){

                        //Toast.makeText(getContext(), "Number Clicked", Toast.LENGTH_LONG).show();
    ...
    private void setTouchSides(){

        int x = mWidth / 2 - mTextDimens.x / 2;

        mLeftRectangle = new Rect(0, 0, x, mHeight);
        mRightRectangle = new Rect(x + mTextBounds.width(), 0, mWidth, mHeight);

        mNumberRectangle = new Rect(x, 0, x + mTextBounds.width(), mHeight);
